# 📘 Lab 8 — Expressions Conditionnelles (AWS CloudShell)
...
locals {
  ...
  common_tags = {
    Lab = "lab8", Username = var.username, Environment = var.environment, ManagedBy = "Terraform"
  }
}
EOF
...
resource "aws_instance" "lab8_instance" {
  ...
  tags = merge(local.common_tags, { Name = "${local.prefix}-instance-${count.index + 1}" })
}
EOF
